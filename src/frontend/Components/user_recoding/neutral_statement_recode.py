...
def _render_neutral_recode_configurator():
    ...
    # Initialize selected neutral questions if not exists
    ...
    for i, (column, label) in enumerate(filtered_questions, 1):
        ...
        if st.session_state.neutral_questions.get(label, {}).get('selected', False):
    ...
    with st.container():
        ...
        with col2:
            if st.button("🗑️", key=f"remove_{index}", help="Remove this question"):
                del st.session_state.neutral_questions[label]
                st.rerun()
